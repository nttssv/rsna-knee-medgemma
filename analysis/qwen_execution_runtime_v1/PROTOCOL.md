...
Before cache audit, tokenizer load, or model load, the runner checks: committed execution-plan bytes/hash; prepared-plan bytes/hash and five-report identity; private authorization and exact plan/proposal hashes; exact pod/GPU/cloud/storage/rates/budget/deadlines; private same-pod stopped-state read/stop/read-back preflight; and a live watchdog whose process, command, environment, receipt, script hash, deadline, and stop-preflight hashes verify. It fails closed on any mismatch. The PID, command line, environment, and receipt hashes are rechecked before every generation; the stopped-pod preflight itself is not repeated.

Before generation it checks the exact single GPU name, records the NVIDIA driver version for provenance without pinning an exact driver build, checks CUDA 12.8, torch 2.8.0+cu128 and all frozen dependency versions, native BF16, at least 36 GiB free GPU memory, the complete pinned offline cache, exact input token parity with the prepared artifacts, input/context caps, SDPA, loaded revision, and Qwen stop IDs. It does not download or fall back to another model, cache, host, or GPU.
