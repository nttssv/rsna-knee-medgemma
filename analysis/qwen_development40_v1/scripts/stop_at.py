"""Exact-pod shutdown backstop for the Qwen development-40 baseline benchmark."""
...
from run_dev40 import (  # noqa: E402
    CONTROL_SHA,
    PREPARED_PLAN_SHA,
    GateError,
    aware,
...
SESSION_FIELDS = {
    "schema_version", "approved_by_user", "execution_plan_sha256", "prepared_plan_sha256",
    "prompt_sha256", "pod_id", "region", "gpu_name", "gpu_count", "cloud_type",
    "container_disk_gb", "persistent_volume_gb", "network_volume_id",
    "compute_usd_per_hour", "storage_usd_per_hour", "maximum_usd", "t0",
    "hard_deadline", "shutdown_at",
}
...
def validate_intent(session: dict, plan_sha: str, pod_id: str, *, worker_due: bool = False,
    ...
    plan = validate_plan(plan_sha)
    if set(session) != SESSION_FIELDS or session.get("schema_version") != 1 \
            or session.get("approved_by_user") is not True:
        raise GateError("Shutdown requires the exact development-40 approval schema")
    if (session.get("execution_plan_sha256") != plan_sha
        or session.get("prepared_plan_sha256") != PREPARED_PLAN_SHA
        or session.get("prompt_sha256") != CONTROL_SHA
        or session.get("pod_id") != pod_id
        or not isinstance(pod_id, str)
        or not re.fullmatch(r"[A-Za-z0-9_-]{1,100}", pod_id)):
        raise GateError("Shutdown intent does not bind the development-40 plan, package and pod")
    t0, deadline, shutdown = map(
        aware, (session["t0"], session["hard_deadline"], session["shutdown_at"])
    )
    ...
    if any(type(x) not in (int, float) or not math.isfinite(x) or x < 0 for x in rates) \
            ...
            or not 0 < ceiling <= plan["max_session_usd"] \
            or not 0 < seconds <= plan["max_provider_seconds"] \
            or sum(rates) * seconds / 3600 > ceiling:
        raise GateError("Shutdown intent exceeds the development-40 budget or duration")
    if shutdown > deadline or (deadline - shutdown).total_seconds() < 300 \
            or now < t0 or (not worker_due and now >= shutdown):
        raise GateError("Development-40 shutdown deadline is invalid or expired")
    return shutdown


def stop_once(pod_id: str, key: str, *, opener=None) -> dict:
    """Issue one exact-pod stop request; opener is injectable for offline CPU tests."""
    ...
    request = urllib.request.Request(API, data=json.dumps({"query": query}).encode(), method="POST",
        headers={"Authorization": "Bearer " + key, "Accept": "application/json",
                 "Content-Type": "application/json",
                 "User-Agent": "rsna-qwen-development40/1.0"})
    if opener is None:
...
def worker(args) -> int:
    ...
    shutdown_at = validate_intent(session, args.plan_sha256, args.pod_id, worker_due=True)
    result = {"pod_id": args.pod_id, "execution_plan_sha256": args.plan_sha256,
              "prepared_plan_sha256": PREPARED_PLAN_SHA, "session_sha256": sha(args.session),
              "hard_deadline": session["hard_deadline"], "synthetic": False}
    while datetime.now(timezone.utc) < shutdown_at:
    ...
    return 0 if result["status"] in (
        "stop_request_accepted", "cancelled_after_operator_verified_stop"
    ) else 1
