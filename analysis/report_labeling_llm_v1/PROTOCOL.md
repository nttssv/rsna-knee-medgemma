...
After explicit resource approval, tokenize all 58 complete rendered prompts with both official model tokenizers before inference. Record the exact rendered text privately, token counts, tokenizer revision and chat templates. MedGemma uses its processor's tokenizing chat-template path in both preflight and inference, preserving processor-specific input fields. No input truncation or unplanned chunking is allowed. Any preflight input beyond 8,192 tokens blocks inference for both models; preserve the failed preflight and explicitly revise the plan before rerunning. Runtime also checks the architecture's total context limit including the 2,048-token output budget; any later overflow (including a longer second-pass prompt) becomes a technical `context_overflow` failure.
