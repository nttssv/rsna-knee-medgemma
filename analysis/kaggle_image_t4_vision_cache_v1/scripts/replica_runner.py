"""Independent V8 replicas with one same-study vision-cache optimization."""
...
def verify_cache_source(request):
    ...
    if sha256_file(path) != request.get('vision_cache_sha256'):
        raise ContractError('Vision cache source checksum mismatch')
...
def _validated_receipt(row, worker_dir, shard, test):
    ...
    cache = result.get('vision_cache', {})
    for key, expected in {'misses': len(shard), 'hits': 11*len(shard),
                          'underlying_vision_calls': len(shard), 'cache_released': True,
...
def run_replicas(request_path, deadline_monotonic, cuda=None):
    ...
    try:
        ...
        study_times = []
        for i in range(2):
            timing = pd.read_csv(root/f'worker_{i}'/'example'/'study_timings.csv')
            values = timing['study_seconds'].to_numpy(float)
            if len(values) != len(shards[i]) or not np.isfinite(values).all() or (values <= 0).any():
                raise ContractError('Invalid complete-study timing evidence')
            study_times.extend(values.tolist())
        target_met = all(seconds < 40.0 for seconds in study_times)
        if time.monotonic() >= deadline_monotonic:
            raise ContractError('Deadline reached before final submission publication')
        os.link(root/'combined_candidate.csv', final)
        status.update(under40_target_met=target_met, target_seconds_strict=40.0,
                      maximum_complete_study_seconds=max(study_times),
                      complete_study_seconds=study_times)
        status.update(status='UNDER40_PARITY_PASS' if target_met else 'PARITY_PASS_TARGET_NOT_MET', final_submission_path=str(final),
...
def worker(request_path, worker_id, worker_dir):
    ...
    try:
        ...
        # Complete-study timing must start cold, including the first study's
        # DICOM processing. The unchanged baseline diagnostic cannot prime cache.
        teacher.images.clear()
        ...
        if (cache_audit['misses'] != len(shard) or cache_audit['hits'] != 11*len(shard)
                or cache_audit['underlying_vision_calls'] != len(shard)
                or not cache_audit['cache_released'] or len(teacher.score_records) != 12*len(shard)):
            raise ContractError('Unexpected cache or twelve-forward accounting')
