...
For HTTP 403 / Cloudflare 1010 during operator checks, follow the [verified API recovery procedure](../../docs/RUNPOD_API_TROUBLESHOOTING.md). The diagnostic must send the application's User-Agent already present in the frozen watchdog; do not infer a failed stop route from an ad hoc probe that omits it.

## Completed A/B session — 25 September 2026, 20:07:48 Singapore

The real 20-generation A40 session completed and was stopped. See [dated results](results/2026-09-25_200748_SGT/RESULTS.md), [aggregate metrics](results/2026-09-25_200748_SGT/aggregate.json), and the [successful session SOP](results/2026-09-25_200748_SGT/SOP.md). Both fixed repeatability gates fail at 58/60 valid-and-identical despite byte-identical repeats. B is not selected for expansion; the next recommendation is target-specific bone-contusion evidence/negation. No new execution is authorized by this record.
