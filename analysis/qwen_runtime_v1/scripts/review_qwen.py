...
def make_bundle(prepared,output,allow_synthetic=False):
    ...
    definitions=(rt.QWEN/'prompts/control_definitions.txt').read_text()
    payload=dict(synthetic=plan['synthetic'],cases=cases,rows=rows,definitions=definitions,
        categories=[c for c in rt.REVIEW_CATEGORIES if c!='reference_discordance']+[TECHNICAL_DISPOSITION])
...
def validate_review(prepared,bundle,review_path,allow_synthetic=False):
    ...
    allowed=(set(rt.REVIEW_CATEGORIES)-{'reference_discordance'})|{TECHNICAL_DISPOSITION}
    for row in rows:
...
def technical_summary(prepared,allow_synthetic=False):
    ...
    for arm in ('control','candidate'):
        ...
        for i,(a,b) in enumerate(zip(first,second)):
            ...
            valid_identical+=sum(x['status']==y['status']=='valid' and all(x[f]==y[f] for f in fields)
                                 for x,y in zip(a['response']['rows'],b['response']['rows']))
            repeat.append(dict(arm=arm,case_index=i,condition_differences=differences))
        gates.append(dict(arm=arm,valid_identical_cells=valid_identical,required=60,passed=valid_identical==60,
                          parser='frozen_v2_primary'))
    for key in rt.ORDER:
        cells=[r for p in runs[key][2] for r in p['secondary_response']]
        secondary.append(dict(run=key,cells=len(cells),statuses=dict(Counter(r['status'] for r in cells)),
                              parser='frozen_v1_sensitivity_not_primary'))
    return dict(status='SYNTHETIC_SOFTWARE_ONLY' if plan['synthetic'] else 'TECHNICAL_ONLY',
        ...
        attempted_generations=20,unrun_generations=0,condition_results=results,repeat_consistency=repeat,
        technical_gates=gates,secondary_parser=secondary,semantic_accuracy=None,expansion_allowed=False)
...
def finalize(prepared,bundle,review_path,output,allow_synthetic=False):
    ...
    # This local runtime never loads organizer references. Separate later stage only.
    context=None
    comparisons=[];counts={}
    ...
    rt.write(output/'organizer_comparison.json',dict(synthetic=data['plan']['synthetic'],review_sha256=data['review_sha256'],
        status='DEFERRED_NO_REFERENCE_LABELS_LOADED',
        note='Blank secondary comparison template. No organizer evaluation performed.',rows=comparisons))
