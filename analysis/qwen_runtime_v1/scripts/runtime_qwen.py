"""Bounded Qwen runtime. GPU execution is disabled; demo is CPU fabrication only."""
import argparse
import csv
...
REPO = ROOT.parents[1]
QWEN = ROOT.parent/'qwen_report_extraction_v1'
V2 = ROOT.parent/'report_labeling_llm_v2'
spec = importlib.util.spec_from_file_location('runtime_qwen_candidate', QWEN/'local_audit.py')
candidate = importlib.util.module_from_spec(spec)
spec.loader.exec_module(candidate)
core = candidate.contract
sys.path.insert(0, str(V2/'scripts'))
from run_smoke import supervise
ORDER = candidate.read(QWEN/'configs/experiment.json')['proposed_order']
REVIEW_CATEGORIES = ['polarity_contradiction','wrong_anatomy_or_compartment','severity_threshold_error',
    'temporality_error','unsupported_negative','other_entailment_failure',
    'insufficient_information','no_issue_identified']
IDENTITY_FIELDS = ('condition','label','confidence','model_evidence','source_span','evidence_start',
                   'evidence_end','evidence_normalization_method','evidence_normalization_applied','status')


def candidate_fingerprints():
    return {str(p.relative_to(QWEN)):core.sha(p) for p in sorted(QWEN.rglob('*'))
            if p.is_file() and '__pycache__' not in p.parts}


def make_prompt(arm, report):
    return candidate.prompt(arm, report)


def validate_primary(raw, report, status='completed'):
    return core.validate_response(raw, report, status)


def validate_secondary(raw, report, status='completed'):
    return candidate.old.validate_response(raw, report) if status=='completed' else candidate.old.failure(status)
...
def fingerprints():
    ...
    paths += [p for folder in ('scripts','configs','tests') for p in (ROOT/folder).rglob('*')
              if p.is_file() and '__pycache__' not in p.parts]
    return {str(p.relative_to(REPO)):sha(p) for p in sorted(paths+[REPO/'tests/test_qwen_runtime.py'])}


def model_config():
    cfg=candidate.read(QWEN/'configs/experiment.json').copy()
    cfg.update(prompt_version='qwen_whole_package_comparison',prompt_file=None,status='UNEXECUTED_RUNTIME')
    return cfg


def guard(execute=False):
    p=policy()
    if not execute or not p['execution_enabled'] or not p['resource_authorization']:
        raise PermissionError('GPU execution disabled; a separately reviewed revision and current resource authorization are required')
    # This published version deliberately has no runtime flag to unlock paid execution.
    raise PermissionError('This local integration revision cannot authorize GPU execution')
...
def prepare(output, source=None, state=None, synthetic=False):
    ...
    if not synthetic and (source is None or state is None):raise ValueError('Verified audit and state required')
    if synthetic:
        ...
        for row in rows:row['report_sha256']=candidate.old.text_sha(row['Report'])
        origin=None
    else:
        source=private(source)
        if sha(source/'receipt.json')!=policy()['audit_receipt_sha256']:raise ValueError('Wrong audited preparation')
        receipt=read(source/'receipt.json')
        for name in ('control_prompts.jsonl','candidate_prompts.jsonl','tokenized.json'):
            if sha(source/name)!=receipt['files'][name]:raise ValueError('Audited prompt/token file changed')
        _,_,_,rows=candidate.verify_inputs(candidate.old.state_dir(state))
        origin=dict(audit_receipt_sha256=sha(source/'receipt.json'),
                    split_sha256=core.config('input_anchors')['split_sha256'])
    output.mkdir(parents=True,mode=0o700)
    with (output/'inputs.jsonl').open('x') as f:
        os.chmod(f.name,0o600)
        for row in rows:append(f,row)
    for arm in ('control','candidate'):
        prompts=[dict(case_index=i,prompt=make_prompt(arm,r['Report'])) for i,r in enumerate(rows)]
        if not synthetic and lines(source/(arm+'_prompts.jsonl'))!=prompts:raise ValueError('Audited prompts differ')
        with (output/(arm+'_prompts.jsonl')).open('x') as f:
            os.chmod(f.name,0o600)
            for p in prompts:append(f,p)
    if not synthetic:
        for name in ('tokenized.json','receipt.json'):
            shutil.copyfile(source/name,output/('audit_receipt.json' if name=='receipt.json' else name))
        shutil.copyfile(Path(state)/'runs/report-labeling-20260913-v1/splits.csv',output/'splits.csv')
        for p in output.iterdir():p.chmod(0o600)
    plan=dict(stage='SYNTHETIC_SOFTWARE_DEMO' if synthetic else 'LOCAL_RUNTIME_PLAN_NOT_RUN',synthetic=synthetic,
        model_calls=0,files={p.name:sha(p) for p in output.iterdir()},runtime_sha256=fingerprints(),
        candidate_sha256=candidate_fingerprints(),policy=policy(),model=model_config(),run_order=ORDER,
        studies=5,maximum_generations=20,planned_condition_cells=240,origin=origin)
...
def checked_plan(prepared,expected_sha):
    ...
    if (plan['runtime_sha256']!=fingerprints() or plan['candidate_sha256']!=candidate_fingerprints()
        ...
        raise ValueError('Runtime plan is stale or outside fixed design')
    required={'inputs.jsonl','control_prompts.jsonl','candidate_prompts.jsonl'}
    if plan['synthetic'] is False:required|={'audit_receipt.json','tokenized.json','splits.csv'}
    elif plan['synthetic'] is not True:raise ValueError('Synthetic must be boolean')
    if set(plan['files'])!=required:raise ValueError('Unexpected prepared files; no historical review context allowed')
    for name,expected in plan['files'].items():
        if sha(prepared/name)!=expected:raise ValueError('Prepared artifact changed')
    rows=lines(prepared/'inputs.jsonl')
    if len(rows)!=5 or len({r['StudyInstanceUID'] for r in rows})!=5:raise ValueError('Expected five unique inputs')
    for row in rows:
        if (set(row)!=candidate.old.INPUT_KEYS or row['split']!='development' or not row['Report']
            or candidate.old.text_sha(row['Report'])!=row['report_sha256']):raise ValueError('Input fingerprint/fields changed')
    for arm in ('control','candidate'):
        if lines(prepared/(arm+'_prompts.jsonl'))!=[dict(case_index=i,prompt=make_prompt(arm,r['Report'])) for i,r in enumerate(rows)]:
            raise ValueError('Prompt changed')
    if plan['synthetic']:
        if plan['origin'] is not None or any(r['Report']!='Synthetic software fixture '+str(i)+'.'
            or r['StudyInstanceUID']!='synthetic-'+str(i) or r['language']!='synthetic' for i,r in enumerate(rows)):
            raise ValueError('Demo only accepts fixed fabricated fixtures')
    else:
        if digest(rows)!=policy()['real_inputs_digest']:raise ValueError('Real input identity/order changed')
        if sha(prepared/'audit_receipt.json')!=policy()['audit_receipt_sha256']:raise ValueError('Unanchored audit')
        receipt=read(prepared/'audit_receipt.json')
        for name in ('control_prompts.jsonl','candidate_prompts.jsonl','tokenized.json'):
            if sha(prepared/name)!=receipt['files'][name]:raise ValueError('Unanchored prompt/token inputs')
        split_sha=core.config('input_anchors')['split_sha256']
        if sha(prepared/'splits.csv')!=split_sha or plan['origin']!={'audit_receipt_sha256':policy()['audit_receipt_sha256'],'split_sha256':split_sha}:
            raise ValueError('Split/origin changed')
        ...
        if len(assignments)!=58 or len(dev)!=40 or sum(r['split']=='validation' for r in assignments)!=18 or not {r['StudyInstanceUID'] for r in rows}<=dev:
...
class SyntheticBackend:
    ...
    def generate(self,encoded):
        ...
        return dict(raw_output=raw,decoded_with_special_tokens=raw,output_token_ids=[8,151645],input_tokens=1,
...
def real_encoder(cache):
    ...
    check_versions()
    encoder=HFEncoder('qwen',cache);encoder.cfg=model_config()
    return encoder


def real_backend(encoder,cache):
    guard(True)  # Also block direct helper calls before importing/loading model weights.
    ...
    backend.metadata.update(effective_qwen_model_config=encoder.cfg,
...
def run_once(prepared,expected_sha,key,cache=None,worker_fd=None,execute=False):
    plan,rows=checked_plan(prepared,expected_sha)
    if key not in ORDER:raise ValueError('Unknown run')
    if not plan['synthetic']:guard(execute)
    ...
    try:
        if plan['synthetic']:
            ...
            audit=audit_cache(cache,'qwen');write(out/'cache_audit.json',audit)
            if not audit['complete']:raise ValueError('Pinned full weight/config cache failed audit')
            encoder=real_encoder(cache)
        entries=[encoder.encode(make_prompt(key.split('-')[0],r['Report'])) for r in rows]
        if not plan['synthetic']:
            expected=read(Path(prepared)/'tokenized.json')[key.split('-')[0]]
            if [dict(rendered_prompt=e.rendered_prompt,input_ids=e.input_ids,input_tokens=e.input_tokens) for e in entries]!=expected:
                raise ValueError('Input tokenization differs from pinned preparation')
        ...
        count=0
        with (out/'raw.jsonl').open('x') as raw,(out/'predictions.jsonl').open('x') as parsed,(out/'attempts.jsonl').open('x') as attempts:
            os.chmod(raw.name,0o600);os.chmod(parsed.name,0o600);os.chmod(attempts.name,0o600)
            for i,(row,encoded) in enumerate(zip(rows,entries)):
                append(attempts,dict(context=ctx,case_index=i,started_at=now()))
                ...
                record=dict(context=ctx,synthetic=plan['synthetic'],case_index=i,StudyInstanceUID=row['StudyInstanceUID'],
                    report_sha256=row['report_sha256'],prompt=make_prompt(key.split('-')[0],row['Report']),
                ...
                response=validate_primary(result['raw_output'],row['Report'],result['generation_status'])
                append(parsed,dict(case_index=i,raw_record_sha256=digest(record),response=response,
                    secondary_response=validate_secondary(result['raw_output'],row['Report'],result['generation_status'])))
                count+=1
...
def verify_run(prepared,key):
    ...
    required={'preflight.json','runtime.json','raw.jsonl','predictions.jsonl','attempts.jsonl'} | (set() if plan['synthetic'] else {'cache_audit.json'})
    ...
    attempts=lines(out/'attempts.jsonl')
    if len(attempts)!=5 or any(a['case_index']!=i or a['context']!=ctx for i,a in enumerate(attempts)):
        raise ValueError('Attempt ledger differs from dispatch/order')
    ...
    for i,(record,pred,row) in enumerate(zip(raw,parsed,rows)):
        ...
        if not plan['synthetic']:
            expected=read(Path(prepared)/'tokenized.json')[key.split('-')[0]][i]
            if record['rendered_prompt']!=expected['rendered_prompt'] or record['input_ids']!=expected['input_ids']:
                raise ValueError('Recorded input differs from pinned tokenization')
        if (record['context']!=ctx or record['case_index']!=i or record['StudyInstanceUID']!=row['StudyInstanceUID']
            ...
            or record['prompt']!=make_prompt(key.split('-')[0],row['Report'])
            or generation['generation_status']!='completed' or pred['case_index']!=i
            or pred['raw_record_sha256']!=digest(record)
            or pred['response']!=validate_primary(generation['raw_output'],row['Report'],'completed')
            or pred['secondary_response']!=validate_secondary(generation['raw_output'],row['Report'],'completed')):
        ...
        if (not record['input_ids'] or any(type(n) is not int or n<0 for n in record['input_ids']+ids)
            ...
            or generation_status(ids,[151645,151643],elapsed,plan['model']['max_time_seconds'],plan['model']['max_new_tokens'])!='completed'
            ...
            generation['output_tokens']>plan['model']['max_new_tokens'] or generation['runtime_seconds']>=300):raise ValueError('Token/time budget mismatch')
    return receipt,raw,parsed


def execute_session(prepared,expected_sha,cache=None,execute=False):
...
def inspect_partial(prepared):
    """Recovery accounting only. No parsing/relabeling of partial model outputs."""
    root=Path(prepared)/'session';start=read(root/'start.json')
    plan,_=checked_plan(prepared,start['plan_sha256'])
    if start['synthetic']!=plan['synthetic'] or start['run_order']!=ORDER:raise ValueError('Parent mismatch')
    result=[]
    for key in ORDER:
        out=root/key;counts={};tails={}
        if out.exists():ctx=context(prepared,key)
        for filename in ('attempts.jsonl','raw.jsonl','predictions.jsonl'):
            path=out/filename;records=[];trailing=False
            if path.exists():
                data=path.read_bytes().splitlines(keepends=True)
                for i,line in enumerate(data):
                    try:row=json.loads(line,object_pairs_hook=core.no_duplicates)
                    except (ValueError,UnicodeDecodeError):
                        if i==len(data)-1 and not line.endswith(b'\n'):trailing=True;break
                        raise ValueError('Corrupt diagnostic ledger')
                    if row.get('case_index')!=len(records):raise ValueError('Diagnostic order/identity mismatch')
                    if filename!='predictions.jsonl' and row.get('context')!=ctx:raise ValueError('Diagnostic dispatch mismatch')
                    records.append(row)
            if len(records)>5:raise ValueError('Excess diagnostic records')
            counts[filename]=len(records);tails[filename]=trailing
        if not counts['attempts.jsonl']>=counts['raw.jsonl']>=counts['predictions.jsonl']:
            raise ValueError('Diagnostic ledger counts inconsistent')
        result.append(dict(key=key,durable_attempt_starts=counts['attempts.jsonl'],raw_records=counts['raw.jsonl'],
            parsed_records=counts['predictions.jsonl'],incomplete_tails=tails,child_receipt_present=(out/'result.json').exists()))
    count=sum(r['durable_attempt_starts'] for r in result)
    return dict(status='DIAGNOSTIC_ONLY_NOT_COMPLETED_VERIFICATION',synthetic=plan['synthetic'],runs=result,
        planned_generations=20,durable_attempt_starts=count,not_recorded_as_started=20-count,
        attempted_definition='Durable dispatch intent; an attempt may fail before entering model.generate.',
        partial_records_are_verified_predictions=False,semantic_accuracy=None,expansion_allowed=False)


def main():
    p=argparse.ArgumentParser(description=__doc__)
    p.add_argument('action',choices=['prepare','demo','run','verify','inspect','_worker'])
    ...
    elif a.action=='inspect':print(json.dumps(inspect_partial(a.prepared),indent=2))
    elif a.action=='run':
        guard(a.execute)
        raise SystemExit(0 if execute_session(a.prepared,a.plan_sha256,a.cache,a.execute) else 1)
    else:
        raise SystemExit(0 if run_once(a.prepared,a.plan_sha256,a.key,a.cache,a.worker_fd,a.execute) else 1)
