"""Package frozen V8 arithmetic with an independent two-process scheduler."""
...
def build(output: Path, reference_csv: Path):
    ...
    cells[0] = original.cell('markdown',
        '# Private T4 dual-replica diagnostic\n\n'
        'Two independent processes, one visible T4 each, using byte-frozen Version 8 '
        'model/adapter/preprocessing/FP16-NF4 and FP32-residual arithmetic. Deterministic '
        'study sharding; twelve prediction forwards per study. No model parallelism, '
        'offload, retry, training or competition submission. The parent compares every '
        'example score against Version 8 only after both workers finish.\n\n'
        'Parity threshold fixed before execution: absolute difference <=1e-6, rtol=0. '
        'Passing this diagnostic establishes THROUGHPUT_READY for replicas, not hidden-test readiness.')
    ...
    cells.append(original.cell('code',
        ...
        "OUT=Path('/kaggle/working')/('t4-replicas-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ'))\n"
        ...
        "print('THROUGHPUT_READY',json.dumps(result),flush=True)\n"))
