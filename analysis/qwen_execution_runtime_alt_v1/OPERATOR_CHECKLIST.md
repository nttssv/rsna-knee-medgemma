...
3. After approving the exact resource selection and within 10 minutes before the start request, capture the exact provider-observation fields plus a timezone-aware `observed_at` and `provider_state: "STOPPED"` in a private `0600` file. Confirm all resource values match the proposed grant exactly. This is pre-start evidence only; it does not claim the pod remains stopped after start.
