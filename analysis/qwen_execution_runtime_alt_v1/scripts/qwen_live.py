...
BASE_RUNTIME = ROOT.parent / "qwen_execution_runtime_v1"
ALT_RESOURCE = ROOT.parent / "qwen_execution_resource_alt_v1"
...
BASE_EXECUTION_PLAN_SHA256 = "0223e5be543e0bb8f4493302476ade6cf53b5ee08653acf42858bd8263e249c5"
PROPOSAL_SHA256 = "884dc38d2004abb44c64d2363e525969bf7f7fabd35f86eb5d03144bf34e72e2"
...
alternate_gate = load_module("qwen_live_alternate_resource_gate", ALT_RESOURCE / "scripts/resource_alt_gate.py")
...
def validate_execution_plan(plan_path, expected_sha256):
    ...
    proposal = read(ALT_RESOURCE / "configs/resource_proposal.json")
    expected_gpus = [item["provider_name"] for item in proposal["gpu_allowlist"]]
    if (expected["prepared_plan_sha256"] != PREPARED_PLAN_SHA256
        or expected["prior_execution_plan_sha256"] != BASE_EXECUTION_PLAN_SHA256
        or expected["resource_proposal_sha256"] != PROPOSAL_SHA256
        or set(expected["gpu_allowlist"]) != set(expected_gpus)
        or len(expected["gpu_allowlist"]) != len(expected_gpus)
        or expected["model_id"] != "Qwen/Qwen3-14B"
        or expected["model_revision"] != "40c069824f4251a91eefaf281ebe4c544efd3e18"
        or expected["precision"] != "bfloat16"
        or expected["attention_implementation"] != "sdpa"
        or expected["batch_size"] != 1
        or expected["reports_per_run"] != 5
        or expected["real_execution_enabled_by_default"] is not False
        or expected["maximum_generations"] != 20
        or expected["run_order"] != ["control-1", "candidate-1", "candidate-2", "control-2"]):
        raise PermissionError("Execution plan scope or reviewed GPU allowlist changed")
    if any(expected[key] is not False for key in
           ("training", "validation_inference", "full_development_inference", "bulk_extraction", "retry_or_repair")):
        raise PermissionError("Scientific scope expansion is not permitted")
    if sha(ALT_RESOURCE / "configs/resource_proposal.json") != expected["resource_proposal_sha256"]:
        raise PermissionError("Alternate resource proposal SHA-256 mismatch")
    if sha(BASE_RUNTIME / "configs/execution_plan.json") != BASE_EXECUTION_PLAN_SHA256:
        raise PermissionError("Reviewed base execution plan SHA-256 mismatch")
    ...
    if sha(ALT_RESOURCE / "scripts/resource_alt_gate.py") != expected["resource_gate_sha256"]:
        raise PermissionError("Alternate resource gate source changed")
    if sha(QWEN_EXECUTION / "scripts/resource_gate.py") != expected["watchdog_gate_sha256"]:
...
def live_gate(plan_path, plan_sha256, authorization, watchdog_receipt, stop_preflight,
              provider_observation, current=None):
...
def validate_user_approval(path, execution_plan_sha256, current=None):
    ...
    grant = read(path)
    proposal_path = ALT_RESOURCE / "configs/resource_proposal.json"
    proposal = read(proposal_path)
    cfg = runtime_config()
    if sha(proposal_path) != PROPOSAL_SHA256:
        raise PermissionError("Approval is not bound to the pinned alternate proposal")
    if grant.get("execution_plan_sha256") != execution_plan_sha256:
        raise PermissionError("Approval is not bound to this exact new execution plan")
    if grant.get("prepared_plan_sha256") != PREPARED_PLAN_SHA256:
        raise PermissionError("Approval is not bound to the frozen prepared plan")
    if grant.get("resource_proposal_sha256") != PROPOSAL_SHA256:
        raise PermissionError("Approval is not bound to the alternate resource proposal")
    # Reuse the reviewed proposal gate for all its resource/time/rate checks. Its schema
    # pins the previous execution plan, so adapt only that in-memory field after the
    # strict new-plan check above; no proposal or on-disk grant is rewritten.
    gate_grant = dict(grant, execution_plan_sha256=proposal["base_execution_plan_sha256"])
    selected = alternate_gate.validate_grant(proposal, gate_grant, PROPOSAL_SHA256,
        resource_gate.parse_time, current=current)
    if grant["gpu_name"] not in {item["provider_name"] for item in proposal["gpu_allowlist"]}:
        raise PermissionError("No fallback: approved GPU is not in the reviewed allowlist")
    if selected["provider_name"] != grant["gpu_name"]:
        raise PermissionError("Approved GPU identity mismatch")
    if grant["execution_plan_sha256"] != execution_plan_sha256:
        raise PermissionError("Execution plan binding changed during validation")
    return grant
...
def verify_worker_attestation(fd, output, execution_plan_sha256, prepared_plan_sha256,
                              authorization, watchdog_receipt, stop_preflight, provider_observation):
    import stat
    ...
    if (message.get("parent_pid") != os.getppid() or start.get("parent_pid") != os.getppid()
        ...
        or start.get("provider_observation_sha256") != sha(provider_observation)
...
def verify_reported_hardware(names, visible_count, cuda_version, torch_version, cuda_available,
                            native_bf16, free_gib, approved_gpu_name):
    if visible_count != 1 or len(names) != 1:
        raise RuntimeError("Exactly one GPU must be visible")
    proposal = read(ALT_RESOURCE / "configs/resource_proposal.json")
    alternate_gate.validate_selected_gpu(proposal, approved_gpu_name, visible_count)
    if names != [approved_gpu_name]:
        raise RuntimeError("Visible GPU differs from the specifically approved GPU")
...
def verify_hardware_and_versions(approved_gpu_name):
...
def create_backend(encoder, cache, approved_gpu_name):
    from v2_runtime import HFBackend
    backend = HFBackend(encoder, cache)
    if backend.metadata["gpu_name"] != approved_gpu_name:
...
def run_session(prepared, prepared_plan_sha256, execution_plan_path, execution_plan_sha256,
                authorization, watchdog_receipt, stop_preflight, provider_observation, cache, output,
    ...
    grant = live_gate(execution_plan_path, execution_plan_sha256, authorization,
                      watchdog_receipt, stop_preflight, provider_observation, current=current)
    output = Path(output).resolve()
    ...
    worker_context = verify_worker_attestation(worker_fd, output, execution_plan_sha256,
        prepared_plan_sha256, authorization, watchdog_receipt, stop_preflight, provider_observation)
    ...
    result = dict(status="failed", prepared_plan_sha256=prepared_plan_sha256,
        ...
        stop_preflight_sha256=sha(stop_preflight), provider_observation_sha256=sha(provider_observation),
        pod_id=grant["pod_id"], gpu_name=grant["gpu_name"], region=grant["region"], run_order=[],
    ...
    try:
        ...
        hw = verify_hardware_and_versions(grant["gpu_name"])
        ...
        grant = live_gate(execution_plan_path, execution_plan_sha256, authorization,
                          watchdog_receipt, stop_preflight, provider_observation)
        backend = create_backend(encoder, cache, grant["gpu_name"])
...
def launch_supervised(prepared, prepared_plan_sha256, execution_plan_path, execution_plan_sha256,
    ...
    start = dict(parent_pid=os.getpid(), prepared_plan_sha256=prepared_plan_sha256,
        ...
        provider_observation_sha256=sha(provider_observation),
    ...
    command = [sys.executable, str(Path(__file__).resolve()), "--execute", "--_worker",
        ...
        "--provider-observation", str(Path(provider_observation).resolve()),
...
def main():
    ...
    if args._worker:
        context = run_session(args.prepared, args.prepared_plan_sha256, args.execution_plan,
            ...
            args.stop_preflight, args.provider_observation, args.cache, args.output, supervisor_context=True,
    ...
    ok = launch_supervised(args.prepared, args.prepared_plan_sha256, args.execution_plan,
        args.execution_plan_sha256, args.authorization, args.watchdog_receipt,
        args.stop_preflight, args.provider_observation, args.cache, args.output)
