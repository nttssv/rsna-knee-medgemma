...
The subsequent [generation candidate](GENERATION_CANDIDATE.md) enables the four configuration gates for a fresh reviewed plan. Descriptions of locked flags below record the original adapter stage; they are superseded only for that candidate. Explicit execution, parent attestation and plan checks remain required. Each real worker now audits full cached bytes against the pinned weight manifest before encoder/backend creation and binds its cache receipt into the run manifest.
...
Exact model revisions remain in the existing [configs](configs/medgemma.json) and [Qwen config](configs/qwen.json). The [runtime policy](configs/runtime.json) pins template hashes, stop-token expectations and package versions. Qwen's template hash was independently checked against its [official pinned tokenizer configuration](https://huggingface.co/Qwen/Qwen3-14B/blob/40c069824f4251a91eefaf281ebe4c544efd3e18/tokenizer_config.json). MedGemma's template/hash and token metadata come from the preserved pinned v1 runtime; both the saved processor and authenticated pinned Hub processor now pass offline tokenization with identical input IDs on the five reports. Its [official model card](https://huggingface.co/google/medgemma-1.5-4b-it) supports the processor-template construction and generated-token slicing used here.
