...
import review_v3_execution as review
...
def completed(tmp_path_factory):
    root=tmp_path_factory.mktemp('v3-demo')/'prepared'
...
def test_history_and_model_recipe():
    assert rt.verify_history()==156
    cfg=rt.model_config()
    assert cfg['max_new_tokens']==4096 and rt.core.config('medgemma')['max_new_tokens']==2048
    assert cfg['model_id']=='google/medgemma-1.5-4b-it'
    assert cfg['revision']==rt.candidate.experiment()['revision']
    assert rt.ORDER==['control-1','candidate-1','candidate-2','control-2']
    assert rt.policy()['execution_enabled']
...
def test_cli_run_refuses_before_inputs(tmp_path):
    result=subprocess.run([sys.executable,str(SCRIPTS/'runtime_v3_execution.py'),'run','--prepared',str(tmp_path/'absent'),'--execute'],capture_output=True,text=True)
...
def test_raw_durable_before_parser_exception(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(rt.candidate,'validate_v3',broken)
    assert not rt.run_once(prepared,expected,rt.ORDER[0],worker_fd=fd)
    out=prepared/'session'/rt.ORDER[0]
