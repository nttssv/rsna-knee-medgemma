...
def build(output: Path) -> dict:
    ...
    cells = [
        ...
        cell(
            ...
            "         'packages':{p:version(p) for p in CONFIG['expected_training_versions']}}\n"
            "free_gib=[torch.cuda.mem_get_info(i)[0]/2**30 for i in range(torch.cuda.device_count())]\n"
            "environment_observed={'runtime':runtime,'free_memory_all_gpus_gib':free_gib,\n"
            " 'timing_seconds':{'asset_verification':ASSET_VERIFY_SECONDS,'dependency_install':DEPENDENCY_INSTALL_SECONDS}}\n"
            "Path('/kaggle/working/environment_observed.json').write_text(json.dumps(environment_observed,indent=2))\n"
            "print('ENVIRONMENT_OBSERVED',json.dumps(environment_observed,indent=2),flush=True)\n"
