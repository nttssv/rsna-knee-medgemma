...
def validate_hardware(devices: list[dict], config: dict) -> dict:
    ...
    first = devices[0]
    if config["require_native_bf16"] and first.get("capability_major", -1) < 8:
        raise ContractError("GPU 0 does not support native BF16")
    if float(first.get("total_gib", 0)) < float(config["minimum_gpu_gib"]):
        raise ContractError("GPU 0 is below the 37 GiB guard; other GPUs are not pooled")
    return {"selected_device": 0, "visible_devices": len(devices), **first}


def validate_runtime_versions(runtime: dict, config: dict) -> dict:
    """Require the frozen pilot runtime instead of silently accepting drift."""
    expected = config["expected_training_versions"]
    if runtime.get("torch") != expected["torch"]:
        raise ContractError(
            f"Torch version drift: expected {expected['torch']}, observed {runtime.get('torch')}"
        )
    if runtime.get("cuda") != expected["cuda"]:
        raise ContractError(
            f"CUDA version drift: expected {expected['cuda']}, observed {runtime.get('cuda')}"
        )
