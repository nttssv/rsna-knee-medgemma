...
ISSUES = []
HEADER_TAGS = ['ImagePositionPatient', 'ImageOrientationPatient', 'InstanceNumber', 'PixelSpacing', 'PatientID', 'Manufacturer', 'ManufacturerModelName', 'MagneticFieldStrength']
...
def rank_series(rows):
    rows = rows.copy()
    rows['_rank'] = pd.to_numeric(rows['Fluid_Sensitive'], errors='coerce').fillna(0) * 2 + pd.to_numeric(rows['Fat_Suppression'], errors='coerce').fillna(0)
