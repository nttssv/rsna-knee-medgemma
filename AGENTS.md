...
- The separate analysis/report_labeling_llm_v2/ is a locally tested candidate with generation NOT RUN. Read its README.md, PROTOCOL.md, RESULTS.md and REVIEW.md. Its bounded v2 adapter has CPU fake-backend tests and a fixed-five dry plan; read RUNTIME.md. Real CPU tokenizer checks are documented in TOKENIZER_PREFLIGHT.md: both pinned Qwen and authenticated pinned MedGemma passed; the earlier saved-export comparison is preserved as historical evidence. Full cached weight checksums and CUDA/BF16 load-only checks passed for both models at executed commit 91bdbbd; read LOAD_PREFLIGHT.md. RunPod was stopped after result transfer. Forward/generation behavior and KV-cache memory remain unverified. The generation candidate enables the four config gates for a fresh reviewed plan; read GENERATION_CANDIDATE.md. This allows the explicit command but does not authorize paid execution. Keep both v1 analysis directories and all historical private outputs unchanged. The v2 private review queue has 12 unresolved development items; synthetic tests and ChatGPT advice are not clinical adjudication.
