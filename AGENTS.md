...
- The separate analysis/report_labeling_llm_v2/ is a locally tested NOT RUN candidate. Read its README.md, PROTOCOL.md, RESULTS.md and REVIEW.md. Its locked v2 adapter has CPU fake-backend tests and a fixed-five dry plan; read RUNTIME.md. Real CPU tokenizer checks are documented in TOKENIZER_PREFLIGHT.md: pinned Qwen passed; saved MedGemma export passed tokenization with revision unverified. CUDA and full model caches remain unverified. All execution flags stay false until a separately reviewed and authorized resource stage. Keep both v1 analysis directories and all historical private outputs unchanged. The v2 private review queue has 12 unresolved development items; synthetic tests and ChatGPT advice are not clinical adjudication.
